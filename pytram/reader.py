...
class Reader(object):
    ...
    """
    def __init__(
            self, files, b_K_i_file=None, kT_file=None, skiprows=0, maxlength=None, verbose=False):
        self.files = files
    ...
    def trajs(self):
        if self._trajs is None:
            ...
            for f in self.files:
                ...
                if (self.maxlength is not None) and (self.maxlength < length):
    ...
    def b_K_i(self):
        if self._b_K_i is None:
            ...
            try:
                ...
                if self.verbose:
                    print("# ... found %d markov and %d thermodynamic states" \
                    % (self._b_K_i.shape[1], self._b_K_i.shape[0]))
            except IOError:
        ...
        if self._kT_K is None:
            ...
            try:
                ...
                if self._kT_K.ndim > 1:
                    if self.verbose:
                        print("# ... found %d columns - restricting to first column" \
                        % self._kT_K.shape[1])
